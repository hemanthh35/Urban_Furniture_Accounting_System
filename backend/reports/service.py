...
def _account_balances(db: Session, account_type: str) -> list[tuple[Account, int, int]]:
...
def balance_sheet(db: Session) -> BalanceSheet:
    ...
    assets = [AccountBalance(account_name=a.name, balance_cents=d - c) for a, d, c in _account_balances(db, "Asset")]
    liabilities = [AccountBalance(account_name=a.name, balance_cents=c - d) for a, d, c in _account_balances(db, "Liability")]
    capital = [AccountBalance(account_name=a.name, balance_cents=c - d) for a, d, c in _account_balances(db, "Capital")]
...
def profit_and_loss(db: Session) -> ProfitAndLoss:
    # Income is credit-normal, Expenses are debit-normal.
    income = [AccountBalance(account_name=a.name, balance_cents=c - d) for a, d, c in _account_balances(db, "Income")]
    expenses = [AccountBalance(account_name=a.name, balance_cents=d - c) for a, d, c in _account_balances(db, "Expense")]
...
def budget_report(db: Session) -> BudgetReport:
    ...
    return BudgetReport(
        rows=[
            BudgetReportRow(
                ...
                analytic_account_name=aa.name, planned_amount_cents=b.planned_amount_cents,
            )
            for b, aa in rows
        ]
    )
